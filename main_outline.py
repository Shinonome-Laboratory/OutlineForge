...
# ---------------------------------------------------------------------------
# Database layer — 拆自本文件（债#2），re-export 保持 main_outline.* 导入兼容
# （tests 与 conftest 均通过 main_outline.get_db / OB_CONFIG_DEFAULTS 访问）。
# ---------------------------------------------------------------------------

from database_outline import (  # noqa: F401
    _CK_PROMPT_CONCEPTS,
    _CK_PROMPT_EDGES,
...
# Background job pool
# ---------------------------------------------------------------------------
# 此前 6 个端点各自每请求现起 threading.Thread，无队列无并发上限（债#3）。
# 统一收敛到一个有界线程池：超出上限的任务排队而非并发挤爆 Ollama / SQLite
# 写锁。上限 4 = 允许 分析 / CKG 抽取 / 教案生成 等不同类任务并行，同类任务
# 靠各自的 per-video / per-job 409 检查防重复提交。
# （03 用 asyncio.Task 单例、01 用单 worker + queue.Queue，模式各异但目的
# 相同；02 的任务函数是同步阻塞式，线程池是改动最小的收敛。）

_JOB_POOL = concurrent.futures.ThreadPoolExecutor(
    max_workers=4, thread_name_prefix="outline-job"
)


def submit_job(fn, *args) -> None:
    """Submit a long-running job to the bounded pool (fire-and-forget).

    任务函数自带 try/except 与 SSE 错误上报；这里仅兜底记录未捕获异常，
    避免 Future 静默吞掉。
    """
    future = _JOB_POOL.submit(fn, *args)

    def _log_uncaught(f):
        exc = f.exception()
        if exc is not None:
            logging.getLogger("outline").error(
                "Background job %s crashed: %s", fn.__name__, exc
            )

    future.add_done_callback(_log_uncaught)
...
async def analyze_video(video_id: int):
    ...
    submit_job(_run_full_analysis, video_id)
...
async def extract_ckg(video_id: int):
    ...
    submit_job(_run_ckg_extraction, video_id)
...
async def get_corpus_ckg():
    ...
    try:
        ...
        # density / avg_path_length / clustering 三列 RETIRED（issue 03）：
        # 2026-07-19 核查 02 前端 / db-viewer / 03 均无读方，payload 已不再
        # 透传；DB 列本身保留（仅旧数据兼容，勿再消费）。
        rows = conn.execute(
            "SELECT c.video_id, v.name AS name, c.model, c.created_at, "
            "c.depth, c.branch_factor, c.convergence_count, "
            "c.bottomup_ratio, c.graph_json, "
...
async def generate_lesson(request: Request):
    ...
    submit_job(_run_lesson_gen, job_id, input_text, mode, chunk_mode)
...
async def lesson_gen_analyze(request: Request):
    ...
    job_id = uuid.uuid4().hex
    _lesson_gen_jobs[job_id] = {"state": "running"}
    submit_job(_run_lesson_gen_analyze, job_id, input_text, chunk_mode)
    return JSONResponse(status_code=202, content={"job_id": job_id})
...
async def lesson_gen_generate_units(request: Request):
    ...
    job_id = uuid.uuid4().hex
    _lesson_gen_jobs[job_id] = {"state": "running"}
    submit_job(_run_lesson_gen_units, job_id, concepts, title, units)
    return JSONResponse(status_code=202, content={"job_id": job_id})
